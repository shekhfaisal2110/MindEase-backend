...
/**
 * Generate insights for a user based on last 30 days of data.
 * Optimised: uses string date comparisons, $lookup with indexed fields,
 * and `allowDiskUse: false` for in‑memory aggregation.
 */
exports.generateInsights = async (userId) => {
  const insights = [];
  ...
  // 1. Sleep vs Next Day Mood – single aggregation with $lookup on string date
  const sleepData = await SleepLog.aggregate([
    { $match: { user: userId, date: { $gte: startStr, $lte: endStr } } },
    {
      $addFields: {
        nextDate: {
          $dateToString: {
            format: "%Y-%m-%d",
            date: { $dateAdd: { startDate: { $dateFromString: { dateString: "$date" } }, unit: "day", amount: 1 } }
          }
        }
      }
    },
    {
      $lookup: {
        ...
        let: { next: "$nextDate", uid: userId },
        pipeline: [
          { $match: { $expr: { $and: [ { $eq: ["$user", "$$uid"] }, { $eq: ["$date", "$$next"] } ] } } },
    ...
    { $unwind: { path: "$mood", preserveNullAndEmptyArrays: true } },
    { $group: { _id: null, avgMoodWellRested: { $avg: "$mood.avgIntensity" } } }
  ...
  // 2. Gratitude Effect on Same‑Day Happiness – optimised
  const gratitudeEffect = await EmotionalActivity.aggregate([
    { $match: { user: userId, date: { $gte: startStr, $lte: endStr } } },
    {
      $lookup: {
        from: 'gratitudeentries',
        let: { d: "$date", uid: userId },
        pipeline: [
          { $match: { $expr: { $and: [ { $eq: ["$user", "$$uid"] }, { $eq: ["$date", "$$d"] } ] } } },
          { $limit: 1 }
        ],
        as: "gratitude"
      }
  ...
  // 3. Screen Time vs Emotional Intensity – using string dates
  const screenVsMood = await DeviceUsage.aggregate([
    { $match: { user: userId, date: { $gte: startStr, $lte: endStr } } },
    {
      $lookup: {
    ...
    { $unwind: { path: "$mood", preserveNullAndEmptyArrays: true } },
    {
      $bucket: {
        ...
        boundaries: [0, 60, 120, 240, 720],
        default: "other",
        output: { avgMood: { $avg: "$mood.intensity" } }
      }
    }
  ], { allowDiskUse: false }).exec();
  const lowScreen = screenVsMood.find(b => b._id === 0)?.avgMood;
  ...
  // 4. Therapy Exercise Consistency – fast count with string date range
  const therapyCount = await TherapyExercise.countDocuments({
    user: userId,
  ...
  // 5. Streak Encouragement – distinct dates using aggregation for speed
  const activeDaysResult = await UserActivity.aggregate([
    { $match: { user: userId, date: { $gte: startStr, $lte: endStr } } },
    { $group: { _id: "$date" } },
    { $count: "total" }
  ], { allowDiskUse: false }).exec();
  const activeDaysCount = activeDaysResult[0]?.total || 0;
  if (activeDaysCount >= 25) {
    insights.push({
      title: '🔥 Outstanding Consistency',
      description: `You've been active on ${activeDaysCount} of the last 30 days! Your dedication is inspiring.`,
      type: 'positive',
      metric: 'streak'
    });
