...
exports.getInsightStats = async (req, res) => {
  try {
    ...
    // Use in‑memory aggregation with `allowDiskUse: false` for speed
    const typeStats = await UserInsight.aggregate([
      { $unwind: "$insights" },
      { $group: { _id: "$insights.type", count: { $sum: 1 } } }
    ...
    const totalInsightsGenerated = await UserInsight.estimatedDocumentCount(); // faster than countDocuments without filter
    const totalUsersWithInsights = await UserInsight.distinct('user').then(users => users.length);
...
exports.refreshInsightStats = async (req, res) => {
  try {
    ...
    statsCache = null;
    cacheTimestamp = null;
    // Re‑fetch and send fresh data
    await exports.getInsightStats(req, res);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};