...
exports.exportProgressPDF = async (req, res) => {
  try {
    ...
    // Convert date strings to Date objects for `updatedAt` filter (used only for Task)
    const startDate = new Date(start);
    const endDate = new Date(end);
    endDate.setHours(23, 59, 59, 999);

    // 1. User info (lean, projection)
    const user = await User.findById(userId)
      .select('username email createdAt')
      .lean();
    user.createdAt = user.createdAt.toISOString().split('T')[0];

    // 2. Points & breakdown – single aggregation using $facet (one round-trip)
    const pointsAgg = await UserActivity.aggregate([
      { $match: { user: userId, date: { $gte: start, $lte: end } } },
      { $group: {
          _id: null,
    ...
    // 3. Activity counts – all in one Promise.all with lean counts
    const [
      affirmationCount,
      gratitudeCount,
      emotionalCount,
      ...
      HourlyEmotion.countDocuments({ user: userId, date: { $gte: start, $lte: end } }),
      ReactResponse.countDocuments({ user: userId, date: { $gte: start, $lte: end } }),
      Task.countDocuments({ user: userId, completed: true, updatedAt: { $gte: startDate, $lte: endDate } }),
      UserActivity.distinct('date', { user: userId, date: { $gte: start, $lte: end } }).then(d => d.length)
    ]);
    ...
    // 4. Emotional stats – single aggregation (avg intensity + top emotion)
    const emotionalAgg = await EmotionalActivity.aggregate([
      { $match: { user: userId, date: { $gte: start, $lte: end } } },
      { $facet: {
          avgIntensity: [{ $group: { _id: null, value: { $avg: "$intensity" } } }],
          emotions: [{ $group: { _id: "$emotion", count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 1 }]
        }
      }
    ], { allowDiskUse: false }).exec();

    let emotional = { intensity: 0, topEmotion: 'N/A' };
    if (emotionalAgg.length) {
      if (emotionalAgg[0].avgIntensity.length) {
        emotional.intensity = emotionalAgg[0].avgIntensity[0].value.toFixed(1);
      }
      if (emotionalAgg[0].emotions.length) {
        emotional.topEmotion = emotionalAgg[0].emotions[0]._id;
      }
    }

    // 5. Sleep stats (single aggregation)
    const sleepAgg = await SleepLog.aggregate([
      { $match: { user: userId, date: { $gte: start, $lte: end } } },
      { $group: { _id: null, avgDuration: { $avg: "$duration" }, avgQuality: { $avg: "$quality" } } }
    ], { allowDiskUse: false }).exec();
    ...
    // 8. Weekly points for chart (last 30 days within period)
    const weeklyPoints = await UserActivity.aggregate([
      { $match: { user: userId, date: { $gte: start, $lte: end } } },
      { $group: { _id: "$date", points: { $sum: "$totalPoints" } } },
      { $sort: { _id: 1 } },
